...
Usage:
python train.py --operator Inverse
nohup python -u train.py --operator RDiff > training_Nonlinear.log 2>&1 &
"""
...
from core.quantum_circuits import generate_simple_hamiltonian, ham_diag_to_operator, generate_ham_spectrum_uniform
...
class OperatorSolver:
    ...
    def __init__(self, operator_type='Inverse', config_file=None, prefix=None):
        ...
        Args:
            operator_type: Operator type ('Inverse', 'Homogeneous', 'Nonlinear', 'RDiffusion', 'Advection', 'Darcy')
    ...
    def load_or_generate_data(self):
        ...
        # --- FIX: Added num_points_0 to filename ---
        num_points_0 = self.config.get('num_points_0', 1000)
        data_file = f"{self.data_dir}/{operator_name}/{operator_name}_Operator_dataset_{self.config['num_train']}_{self.config['num_test']}_{self.config['num_points']}_{num_points_0}_{self.config['train_sample_num']}_{self.config['test_sample_num']}.npz"
        # -------------------------------------------
    ...
    def generate_data(self):
        ...
        train_input = ms.Tensor(np.concatenate((train_branch_input, train_trunk_input), axis=1), ms.float32)
        test_input = ms.Tensor(np.concatenate((test_branch_input, test_trunk_input), axis=1), ms.float32)

        # Convert outputs to tensors
        train_output = ms.Tensor(train_output, ms.float32)
        test_output = ms.Tensor(test_output, ms.float32)

        # Convert branch/trunk inputs to tensors
        train_branch_input = ms.Tensor(train_branch_input, ms.float32)
        train_trunk_input = ms.Tensor(train_trunk_input, ms.float32)
        test_branch_input = ms.Tensor(test_branch_input, ms.float32)
        test_trunk_input = ms.Tensor(test_trunk_input, ms.float32)
        ...
        # --- FIX: Included num_points_0 in filename ---
        num_points_0 = self.config.get('num_points_0', 1000)
        data_file = f"{self.data_dir}/{operator_name}_Operator_dataset_{self.config['num_train']}_{self.config['num_test']}_{self.config['num_points']}_{num_points_0}_{self.config['train_sample_num']}_{self.config['test_sample_num']}.npz"
        # ----------------------------------------------
        ...
        # Save configuration
        config_file = f"{self.data_dir}/{operator_name}_Operator_config_{self.config['num_train']}_{self.config['num_test']}_{self.config['num_points']}_{num_points_0}_{self.config['train_sample_num']}_{self.config['test_sample_num']}.json"
    ...
    def get_qubits_folder(self):
        """Return qubits folder name like qubits3"""
    ...
    def create_model(self):
        ...
        if self.config.get('ham_diag', None) is None:
            ham = generate_simple_hamiltonian(self.config['num_qubits'], lower_bound=self.config['ham_bound'][0], upper_bound=self.config['ham_bound'][1], pauli=self.config['ham_pauli'])
        else:
            diag = self.config['ham_diag']
    ...
    def train_model(self):
        ...
        # Get batch size
        batch_size = self.config.get('batch_size', len(train_input))  # Default to all data
        ...
        for epoch in tqdm(range(self.config['num_epochs']), desc="Training Progress"):
            ...
            for batch_idx in range(num_batches_train):
                ...
                #     batch_branch = batch_input[0]
                #     batch_trunk = batch_input[1]
                # else:
                #     # Compatible with old logic, if not tuple, may need adjustment in dataset processing phase
                #     # Here we assume your DeepONet input must be (branch, trunk)
                #     raise ValueError("Batch Input must be a tuple of (branch, trunk)")

                # # 2. Prepare boundary data (BC)
                # # Call the helper function just added
                # batch_input_bc, batch_target_bc = self.prepare_bc_batch(batch_branch, batch_trunk)

                # # 3. Execute training step

                # train_loss = train_net(batch_input, batch_input_bc, batch_target_bc, batch_output)
    ...
    def save_model(self, suffix="", overwrite=False):
        ...
        """
        self.checkpoints_dir = os.path.join(self.prefix, "checkpoints") if self.prefix else "checkpoints"
        self.checkpoints_dir = os.path.join(self.checkpoints_dir, self.operator_type, self.describe)
        os.makedirs(self.checkpoints_dir, exist_ok=True)
        if suffix == "best" and overwrite:
            # Best model: fixed filename, overwrite save
            operator_name = self.operator_type
            filename = f"best_{self.describe}.ckpt"
            ...
            operator_name = self.operator_type
            filename = f"{suffix}_{self.describe}.ckpt"
            filepath = os.path.join(self.checkpoints_dir, filename)
            dir_name = os.path.dirname(filepath)
            if dir_name and not os.path.exists(dir_name):
                os.makedirs(dir_name, exist_ok=True)
    ...
    def run_complete_pipeline(self):
        ...
        if self.config['ham_diag']:
            self.describe = f"{self.operator_type}_{self.model_name}_{self.config['num_train']}*{self.config['train_sample_num']}_{self.config['num_qubits']}_{self.config['ham_pauli']}_{self.config['ham_bound']}_{self.config['ham_diag']}_{self.config['net_size']}_{self.config['scale_coeff']}_{self.config['random_seed']}"
        else:
            self.describe = f"{self.operator_type}_{self.model_name}_{self.config['num_train']}*{self.config['train_sample_num']}_{self.config['num_qubits']}_{self.config['ham_pauli']}_{self.config['ham_bound']}_{self.config['net_size']}_{self.config['scale_coeff']}_{self.config['random_seed']}"
        try:
            # 1. Data preparation
            ...
            if self.config['if_train']:
                ...
                if self.best_model_path and os.path.exists(self.best_model_path):
                    print(f"\nLoading best model for evaluation from: {self.best_model_path}")
                    # 1. Load checkpoint file to get parameter dictionary
                    param_dict = load_checkpoint(self.best_model_path)
                    # 2. Load parameters into current model instance
                    load_param_into_net(self.model, param_dict)
                else:
                    print("\nWarning: No best model found or file missing. Using final epoch weights.")
...
def main():
    ...
    parser.add_argument('--num_points', type=int, help='Number of sensors')
    # --- FIX: Added missing argument ---
    parser.add_argument('--num_points_0', type=int, help='Number of input resolution points')
    # -----------------------------------
    parser.add_argument('--num_cal', type=int, help='Number of calculating points')
    ...
    parser.add_argument('--ham_diag', type=float, nargs='+', help='Hamiltonian diagnal elements (only used when operator=Inverse(ODE))')
    parser.add_argument('--ham_pauli', type=str, help='Hamiltonian Pauli Operator')
    parser.add_argument('--ham_bound', type=int, nargs='+', help='Bound of Hamiltonian')
    ...
    if args.num_points is not None:
        solver.config['num_points'] = args.num_points
    # --- FIX: Apply override ---
    if args.num_points_0 is not None:
        solver.config['num_points_0'] = args.num_points_0
    # ---------------------------
    if args.num_cal is not None:
        solver.config['num_cal'] = args.num_cal
    ...
    if args.ham_diag is not None:
        solver.config['ham_diag'] = args.ham_diag
    if args.ham_pauli is not None:
        solver.config['ham_pauli'] = args.ham_pauli
    if args.ham_bound:
        solver.config['ham_bound'] = args.ham_bound
