...
for seed_num in seeds:
    for model_type in model_types:
        for operator in operators:
            print(f"Running {operator} with, seed {seed_num}, model {model_type}")
            ...
            if DE_dict[operator] == "PDE":
                ...
                net_size_dict = {"DeepONet":(3, 15, 3, 15), "FNN":(3, 16), "DeepONet_nobias":(3, 15, 3, 15)}
            else:
                num_train=1000
                train_sample_num=10
                num_test=1000
                test_sample_num=100
                net_size_dict = {"DeepONet":(2, 10, 2, 10), "FNN":(2, 10), "DeepONet_nobias":(2, 10, 2, 10)}
            ...
            if if_save:
                ...
                if if_save:
                    ...
                    if model_type == "DeepONet":
                        ...
                        # trunk_size[-1] = 32
                        net = dde.nn.DeepONet(branch_size, trunk_size, "tanh", "Glorot normal",)
                    elif model_type == "DeepONet_nobias":
                        branch_size = [train_branch_input.shape[1]]
                        for _ in range(net_size[0]):
                            branch_size.append(net_size[1])
                        # branch_size[-1] = 32
