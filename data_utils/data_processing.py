...
def sample_2D_Operator_data(u0, u, x, t, sample_num):
    ...
    for i in range(num):
        indices = ms.Tensor(np.random.choice(num_sensors**2, sample_num, replace=False), ms.int32)
        gather = P.Gather()
        x_repeat = ops.expand_dims(mnp.repeat(x, num_sensors, axis=0), 1)
        t_tile = ops.expand_dims(mnp.tile(t, num_sensors), 1)
...
def sample_1D_Operator_data(u0, u, x, sample_num):
    ...
    for i in range(num):
        indices = ms.Tensor(np.random.choice(num_sensors, sample_num, replace=False), ms.int32)
        gather = P.Gather()
...
def ODE_encode(generate_data, num_train, num_test, num_sensors, train_sample_num, test_sample_num):
    ...
    train_u0, train_u, test_u0, test_u, x = generate_data(num_train, num_test, num_sensors)
